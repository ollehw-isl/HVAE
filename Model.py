...
#    return torch.mean(torch.reciprocal(torch.exp(weight)) * ((input - target) ** 2), dim = 1)

# %%
#def loss_function(beta, recon_x, recon_sigma, x, mu, logvar):
#    Recon_error = weighted_mse_loss(recon_x, x, recon_sigma) + torch.sum(torch.abs(recon_sigma), dim = 1)
#    eps = 1e-16
#    # 0.5 * sum(1 + log(sigma^2) - mu^2 - sigma^2)
#    KLD = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp(), dim = 1)
#    loss = (beta * Recon_error + eps) + KLD
#    return loss.mean()
